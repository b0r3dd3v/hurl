...
fn download_chapter(
    ...
    let mut page_vec : Vec<String> = chapter_data.page_array.clone();
    page_vec.sort_unstable_by_key(|k| extension(k.clone()));
    for (pagenum, page) in page_vec.into_iter().enumerate() { // .iter().enumerate() and (num, page) 4 sorted array. Fix serde inst of sort.
        let mut page_name = format!("{:0>4}", pagenum);
